---
title: 'Section Data'
subtitle: Pre-Test Summary; Reach (Pre/Post Test)
...
pptengl2.df <- rawppt2.df[2:nrow(rawppt2.df),c(3,11:69,77:80,86,87,89:93)] #72 variables for IN-clued
...
pptengl.df <- rawppt.df[2:nrow(rawppt.df),c(3,12:70,78:80,83,110:116)] #99 variables for MPC
...
ppt.df <- rbind(pptengl.df,pptengl2.df)
...
table(ppt.df$`Sexual Identity`) #check against Survey Monkey's Question Summary
...
ggsave("Elon - Experiences - Pre.png", width = 9, height = 2.5)
...
###########################
gpthoughts.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup,c(72,4,6,67:71)] #5 Thought variables (likert) for Pre AND Post
...
ggsave("Elon - Thoughts over time.png", width = 9, height = 4.5)
...
gpsatisfaction.df <- ppt.df[ppt.df$`Group Name`==thisgroup & ppt.df$Time=="After",c(72,49:56)] #8 Group variables on Post, excluding comments
...
gpdemographics.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(72,4,3,6,10:15,17,18,20:24,26)] #8 Demographic variables
