...
ppt.df <- rawppt.df[2:nrow(rawppt.df),c(3,11:69,77:115)] #99 variables
names(ppt.df) <- c("Start Date","Initials","Birthdate",
...
ppt.df[68:85] <- lapply(ppt.df[68:85], cleanIt) #both: if used correctly Qs
...
library(likert)
ptexperlkt.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(28:33)] #6 Experience variables (likert)
...
###########################
ptthoughts.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup,c(4,6,95:99)] #5 Thought variables (likert) for Pre AND Post
...
ggsave("PAYC071221b - Thoughts over time.png", width = 9, height = 4.5)
