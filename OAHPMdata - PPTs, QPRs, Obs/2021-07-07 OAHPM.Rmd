...
# double-check column numbers for subsets because you added a start date column
...
ggsave("DreamCenter102621 - Experiences - Pre.png", width = 9, height = 2.5)

########################

ptexpermrd.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(34,36:47)] #13 Experience options (MR dummy excluding duplicate column and "Other method" (text))
...
###########################
ptthoughts.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup,c(100,4,6,95:99)] #5 Thought variables (likert) for Pre AND Post
ptthoughts.df <- na.omit(ptthoughts.df) 

thoughtlkt <- likert(ptthoughts.df[,4:8], grouping = ptthoughts.df$Time)
plot(thoughtlkt, group.order = c("Before","After")) + ggtitle("Participants' Thoughts about Future Sexual Activity")
ggsave("DreamCenter102621 - Thoughts over time.png", width = 9, height = 4.5)
...
ptsatisfaction.df <- ppt.df[ppt.df$`Group Name`==thisgroup & ppt.df$Time=="After",c(100,49:56)] #8 Group variables on Post, excluding comments
...
ptdemographics.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(4,100,3,9:15,17,18,20:24,26)] #8 Demographic variables 
...
ptdemographics.df$gender_cat <- apply(ptdemographics.df[13:17], 1, function(x) {ifelse(sum(x) > 1, "Multigender", names(x[x != 0]))})
gender <- data.frame(table(ptdemographics.df$gender_cat))
names(gender) <- c("Demographics","Count")

ptdemographics.df$race_cat <- apply(ptdemographics.df[5:10], 1, function(x) {ifelse(sum(x) > 1, "Multiracial", names(x[x != 0]))})
...
library(sjPlot)
tab_df(reach,
       title = "Reach for: DreamCenter102621",
       file = "DreamCenter102621 - Demographics Table.doc")
oahrace <- tab_xtab(var.row = ptdemographics.df$race_cat, var.col = ptdemographics.df$Hispanic,
         title = "Race by Ethnicity for: DreamCenter102621",
         file = "DreamCenter102621 - OAH Race by Ethnicity Table.doc") #may not print if no Hispanic YP
