...
rawpptm.df <- read.csv("Participant Pre  Post Test Survey MPC.csv", stringsAsFactors = TRUE)
# OR
rawppti.df <- read.csv("Participant Pre  Post Test Survey IN-clued.csv", stringsAsFactors = TRUE)
...
pptengli.df <- rawppti.df[2:nrow(rawppti.df),c(3,11:69,77:80,86,87,89:93)] #71 variables for IN-clued + MPC
names(pptengli.df) <- c("Start Date","Initials","Birthdate",
...
pptenglm.df <- rawpptm.df[2:nrow(rawpptm.df),c(3,12:70,78:80,83,110:116)] #71 variables for MPC + IN-clued
names(pptenglm.df) <- c("Start Date","Initials","Birthdate",
                   "Group Name", "Participant", "Time",
                   "Alamance County",
                   "Other County (text)",
                   "Grade",
...
pptspanm.df <- rawpptm.df[2:nrow(rawpptm.df),c(3,118:176,184:186,189,216:222) ] #99 variables
names(pptspanm.df) <- c("Start Date","Initials","Birthdate",
                   "Group Name", "Participant", "Time",
                   "Alamance County",
                   "Other County (text)",
                   "Grade",
...
#                   "Water-based lubricants",    #col 215
                   ...
                   "Prepared to use birth control",
                   "Prepared to go to a doctor for birth control")
pptspanm.df[pptspanm.df==""] <- NA

ppt.df <- rbind(pptengli.df,pptenglm.df,pptspanm.df)
...
gpexpermrd.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(72,4,34,36:47)] #13 Experience options (MR dummy excluding duplicate column and "Other method" (text))
...
reach <- rbind.data.frame(gpdgender,gpdrace,gpdage,gpdgrade)
table(gpexpermrd.df$`Never had sex`==1)

mrdnever   <- data.frame(table(ppt.df$`Never had sex`))
...
mrdall <- rbind.data.frame(mrdnever,mrdnobm,mrdnobc,mrdec,mrdiud,mrdimplant,mrdshot,mrdring,mrdpatch,mrdpill,mrdbm,mrdns)
...
yrlktthought <- likert(yrptthoughts.df[,4:8], grouping = yrptthoughts.df$Time)
...
gender <- data.frame(table(yrdemographics.df$gender_cat))
...
race <- data.frame(table(yrdemographics.df$race_cat))
names(race) <- c("Demographics","Count")

age <- data.frame(table(yrdemographics.df$Age))
names(age) <- c("Demographics","Count")

grade <- data.frame(table(yrdemographics.df$Grade))
