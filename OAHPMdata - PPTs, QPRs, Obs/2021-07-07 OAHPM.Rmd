...
# OR
rawppt2.df <- read.csv("Participant Pre  Post Test Survey IN-clued.csv", stringsAsFactors = TRUE)

#English Pre/Post-Tests: subsetted dataframe for IN-clued
pptengl2.df <- rawppt2.df[2:nrow(rawppt2.df),c(3,11:69,77:80,86:93)] #72 variables for IN-clued
names(pptengl2.df) <- c("Start Date","Initials","Birthdate",
...
pptengl2.df[pptengl2.df==""] <- NA

#English Pre/Post-Tests: subsetted dataframe for MPC
pptengl.df <- rawppt.df[2:nrow(rawppt.df),c(3,12:70,78:116)] #99 variables for MPC
...
#English Pre/Post-Tests: Identifiers (repeat for IN-clued, if needed)
...
pptengl.df[10:15] <- lapply(pptengl.df[10:15], cleanIt) #pre: racial identity options (repeat for IN-clued, if needed)
pptengl.df[20:24] <- lapply(pptengl.df[20:24], cleanIt) #pre: gender identity options (repeat for IN-clued, if needed)
pptengl.df[34:47] <- lapply(pptengl.df[34:47], cleanIt) #pre: last time sex options   (repeat for IN-clued, if needed)
pptengl.df[68:85] <- lapply(pptengl.df[68:85], cleanIt) #both: if used correctly Qs   (MPC only)
...
#English Pre-Tests: Categorical Variables - Demographics (repeat for IN-clued, if needed)
...
library(likert)
gpexperlkt.df <- pptengl.df[pptengl.df$`Group Name` %like% thisgroup & pptengl.df$Time=="Before",c(28:33)] #6 Experience variables (likert)
...
ggsave("Elon111421 - Experiences - Pre.png", width = 9, height = 2.5)

########################

#Optional: subset the types of past sexual experiences

gpexpermrd.df <- pptengl.df[pptengl.df$`Group Name` %like% thisgroup & pptengl.df$Time=="Before",c(34,36:47)] #13 Experience options (MR dummy excluding duplicate column and "Other method" (text))
#how to make a frequency table of mrd?

#tab_stackfrq(gpexpermrd.df) #percents by response
...
###########################
gpthoughts.df <- pptengl.df[pptengl.df$`Group Name` %like% thisgroup,c(73,4,6,68:72)] #5 Thought variables (likert) for IN-clued Pre AND Post
gpthoughts.df <- na.omit(gpthoughts.df) 

gpthoughts.df <- pptengl.df[pptengl.df$`Group Name` %like% thisgroup,c(100,4,6,95:99)] #5 Thought variables (likert) for MPC Pre AND Post
gpthoughts.df <- na.omit(gpthoughts.df) 
...
ggsave("Elon111421 - Thoughts over time.png", width = 9, height = 4.5)
...
ggsave("Elon111421 - Thoughts over time.png", width = 9, height = 2.5)
#may not plot if too many listwise deletions
...
gpsatisfaction.df <- pptengl.df[pptengl.df$`Group Name`==thisgroup & pptengl.df$Time=="After",c(73,49:56)] #8 Group variables on IN-clued Post, excluding comments
gpsatisfaction.df <- na.omit(gpsatisfaction.df)

gpsatisfaction.df <- ppt.df[ppt.df$`Group Name`==thisgroup & ppt.df$Time=="After",c(100,49:56)] #8 Group variables on MPC Post, excluding comments
gpsatisfaction.df <- na.omit(gpsatisfaction.df)
...
gpdemographics.df <- pptengl.df[pptengl.df$`Group Name` %like% thisgroup & pptengl.df$Time=="Before",c(4,73,3,9:15,17,18,20:24,26)] #8 Demographic variables for IN-clued
gpdemographics.df <- na.omit(gpdemographics.df)

gpdemographics.df <- pptengl.df[pptengl.df$`Group Name` %like% thisgroup & pptengl.df$Time=="Before",c(4,100,3,9:15,17,18,20:24,26)] #8 Demographic variables for MPC
gpdemographics.df <- na.omit(gpdemographics.df)
...
library(sjPlot)
tab_df(reach,
       title = "Reach for: Elon111421",
       file = "Elon111421 - Demographics Table.doc")
oahrace <- tab_xtab(var.row = gpdemographics.df$race_cat, var.col = gpdemographics.df$Hispanic,
         title = "Race by Ethnicity for: Elon082421",
         file = "Elon - OAH Race by Ethnicity Table.doc") #may not print if no Hispanic YP
# OR
