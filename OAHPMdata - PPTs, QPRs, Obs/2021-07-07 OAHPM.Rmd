...
# 8/06F: You added a "not sure" option to each of the knowledge questions. That will impact the factor levels and labels. You moved the DOB question to the assent form, which will impact the columns and names of the data frame. 
...
ppt.df <- rawppt.df[2:nrow(rawppt.df),c(3,12:69,77:115)] #98 variables
names(ppt.df) <- c("Start Date","Group Name", "Participant", "Time",
                   "Alamance County",
                   "Other County (text)",
                   "Birthdate",
                   "Grade",
...
ppt.df[9:14]  <- lapply(ppt.df[9:14],  cleanIt) #pre: racial identity options
ppt.df[19:23] <- lapply(ppt.df[19:23], cleanIt) #pre: gender identity options
ppt.df[33:46] <- lapply(ppt.df[33:46], cleanIt) #pre: last time sex options
ppt.df[67:84] <- lapply(ppt.df[67:84], cleanIt) #both: if used correctly Qs
...
library(likert)
ptexperlkt.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(27:32)] #6 Experience variables (likert)
...
ptexpermrd.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(33,35:46)] #13 Experience options (MR dummy excluding "Other method" (text))
...
###########################
ptthoughts.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup,c(2,4,94:98)] #5 Thought variables (likert) for Pre AND Post
...
ptsatisfaction.df <- ppt.df[ppt.df$`Group Name`==thisgroup & ppt.df$Time=="After",c(2,48:58)] #11 Group variables on Post
...
ptksa.df <- ppt.df[ppt.df$`Group Name`%like% thisgroup,c(2:4,62:93)] #16 KSA variables with 32 options
...
yrptdemographics.df <- ppt.df[ppt.df$`Start Date` >=begin & ppt.df$`Start Date`<=finish & ppt.df$Time=="Before",c(2,4,5,7:14,16,17,19:23,25)] #8 Demographic variables 
yrptdemographics.df <- na.omit(yrptdemographics.df)
...
yrptdemographics.df$gender_cat <- apply(yrptdemographics.df[14:18], 1, function(x) {ifelse(sum(x) > 1, "Multigender", names(x[x != 0]))})
gender <- data.frame(table(yrptdemographics.df$gender_cat))
names(gender) <- c("Demographics","Count")

yrptdemographics.df$race_cat <- apply(yrptdemographics.df[6:11], 1, function(x) {ifelse(sum(x) > 1, "Multiracial", names(x[x != 0]))})
...
library(sjPlot)
tab_df(reach,
       title = "Reach for Year 1",
       file = "Year 1 - Demographics Table.doc")

################################################

library(likert)
yrptexperlkt.df <- ppt.df[ppt.df$`Start Date` >=begin & ppt.df$`Start Date` <=finish & ppt.df$Time=="Before",c(27:32)] #6 Experience variables (likert)
...
yrptthoughts.df <- ppt.df[ppt.df$`Start Date`>=begin & ppt.df$`Start Date`<=finish,c(2,4,94:98)] #5 Thought variables (likert) for Pre AND Post
yrptthoughts.df <- na.omit(yrptthoughts.df) 

yrthoughtlkt <- likert(yrptthoughts.df[,3:7], grouping = yrptthoughts.df$Time)
...
yrptsatisfaction.df <- ppt.df[ppt.df$`Start Date`>=begin & ppt.df$`Start Date`<=finish & ppt.df$Time=="After",c(2,48:58)] #11 Group variables on Post
...
ptdemographics.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(2,4,5,7:14,16,17,19:23,25)] #8 Demographic variables 
