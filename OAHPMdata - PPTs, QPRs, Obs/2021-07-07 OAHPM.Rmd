...
yroahrace <- tab_xtab(var.row = yrptdemographics.df$race_cat, var.col = yrptdemographics.df$Hispanic,
         title = "Race by Ethnicity for: 2020-2021",
         file = "2020-2021 - OAH Race by Ethnicity Table.doc") #may not print if no Hispanic YP
...
ptdemographics.df <- ppt.df[ppt.df$`Group Name` %like% thisgroup & ppt.df$Time=="Before",c(100,4,9:15,17,18,20:24,26)] #8 Demographic variables 
...
ptdemographics.df$gender_cat <- apply(ptdemographics.df[12:16], 1, function(x) {ifelse(sum(x) > 1, "Multigender", names(x[x != 0]))})
gender <- data.frame(table(ptdemographics.df$gender_cat))
names(gender) <- c("Demographics","Count")

ptdemographics.df$race_cat <- apply(ptdemographics.df[4:9], 1, function(x) {ifelse(sum(x) > 1, "Multiracial", names(x[x != 0]))})
