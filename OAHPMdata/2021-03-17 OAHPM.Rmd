...
```{r}
ppt.df <- rawppt.df[2:nrow(rawppt.df),c(12:18,20:69,77:115)] #96 variables
...
ppt.df[ppt.df==""] <- NA
ppt.df$Hispanic[ppt.df$Hispanic=="0" | ppt.df$Hispanic=="2"] <- NA #to convert categorical to dummy
...
```{r}
#Pre/Post Tests
ppt.df[8:12] <- lapply(ppt.df[8:12], cleanIt) #racial identity options
ppt.df[14] <- lapply(ppt.df[14],cleanIt)
ppt.df[17:21] <- lapply(ppt.df[17:21], cleanIt) #gender identity options
ppt.df[31:45] <- lapply(ppt.df[31:45], cleanIt) #last time sex options
ppt.df[64:82] <- lapply(ppt.df[64:82], cleanIt) #if used correctly Qs
...
- Labels for each categorical response
```{r}
#Pre/Post Tests
...
#Fidelity Log/Quarerly Program Reports
labeltool <- c("P/PT","FL","QPR")
...
demographics.df <- ppt.df [ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="Before",c(1:24,97)] 
experiences.df <- ppt.df [ppt.df$`Group Name`=="payc022521",c(1:3,25:45)]
thoughts.df <- ppt.df[ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1:3,92:96)] 
ksa.df <- ppt.df [ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1:3,60:91)] 
satisfaction.df <- ppt.df [ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="After",c(1:3,46:56)] 

#Referrals by IO for the reporting period:
referrals.df <- flqpr.df[flqpr.df$tool=="FL" & flqpr.df$`FL grp` %like% "payc" & flqpr.df$`QPR period` %like% "Y1", c(3:15,32:46)] #change value of the group name, period, and EBP columns  
...
#Quarterly Program reports
QPR.df <- flqpr.df[flqpr.df$tool=="QPR" & flqpr.df$`QPR period` %like% "Y1",c(87:141)] #change value of reporting period
```
...
mrdumgender <- data.frame(Freq=colSums(demographics.df[17:21]))
mrdumrace <- data.frame(Freq=colSums(demographics.df[8:12]))
mrdumhisp <- data.frame(Freq=colSums(demographics.df[14]))
mrdumlastsex <- data.frame(Freq=colSums(experiences.df[10:24]))
