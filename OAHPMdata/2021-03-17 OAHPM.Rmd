---
title: "Section: Reach (P/PT & QPR data)"
subtitle: "Summarizing Reach by Group"
...
```{r}
ppt.df <- rawppt.df[2:nrow(rawppt.df),c(12:19,20:69,77:115)] #97 variables
...
#will convert categorical (0=not Hispanic; 1=Hispanic; 2=prefer not to answer) to dummy with cleanIt function
...
```{r}
#Pre/Post Tests
ppt.df[8:13] <- lapply(ppt.df[8:13], cleanIt) #racial identity options
ppt.df[15] <- lapply(ppt.df[15],cleanIt)
ppt.df[18:22] <- lapply(ppt.df[18:22], cleanIt) #gender identity options
ppt.df[32:45] <- lapply(ppt.df[32:45], cleanIt) #last time sex options
ppt.df[66:83] <- lapply(ppt.df[66:83], cleanIt) #if used correctly Qs
...
# P/PT Group satisfaction codebook: 1=disagree; 2=not sure; 3=agree; 4=prefer not to answer (replaced with NA)
...
experiences.df <- ppt.df [ppt.df$`Group Name`=="payc022521",c(1:3,26:46)]
thoughts.df <- ppt.df[ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1:3,93:97)] 
ksa.df <- ppt.df [ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1:3,61:92)] 
satisfaction.df <- ppt.df [ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="After",c(1:3,47:57)] 

#IO's referrals for every group in the reporting period (change values of the EBP dates):
referrals.df <- flqpr.df[flqpr.df$`FL io` =="PAYC" & flqpr.df$`MPC1 start` >="2021-01-01" & flqpr.df$`MPC1 start` <="2021-03-31",c(2:3,9:15,33)] #NAed the data
referrals.df <- na.omit(referrals.df)

#Sessions completed as planned in the reporting period (change values of the EBP dates):
fidelity.df <- flqpr.df[flqpr.df$`MPC1 start` >="2021-01-01" & flqpr.df$`MPC1 start` <="2021-03-31",c(1:3,34,39,44)]
...
#mrdumgender <- data.frame(Freq=colSums(demographics.df[18:22]))
#mrdumrace <- data.frame(Freq=colSums(demographics.df[8:13]))
#mrdumhisp <- data.frame(Freq=colSums(demographics.df[15]))
mrdumlastsex <- data.frame(Freq=colSums(experiences.df[10:23]))

# how to combine with different number of columns? will need to manually create tables for MRDUMs
#gender <- data.frame(Demographics=c("Male","Female","Transgender","Nonbinary/Does not identify","Gender not reported"),Freq=c(6,4,0,0,0))
#race <- data.frame(Demographics=c("White","Black","Asian","Native American", "Pacific Islander","Multiracial", "Race not Reported","Hispanic"),Freq=c(0,10,0,0,0,1,0,0))

gender.df <- ppt.df[ppt.df$`Group Name`=="payc022521",c(18:22)]
gender.df$gender_cat <- apply(gender.df, 1, function(x) {ifelse(sum(x) > 1, "Other Gender", names(x[x != 0]))})
gender <- data.frame(table(gender.df$gender_cat))
names(gender) <- c("Demographics","Freq")

race.df <- ppt.df[ppt.df$`Group Name`=="payc022521",c(8:13)]
race.df$race_cat <- apply(race.df, 1, function(x) {ifelse(sum(x) > 1, "Multiracial", names(x[x != 0]))})
race <- data.frame(table(race.df$race_cat))
names(race) <- c("Demographics","Freq")

demographics.df <- ppt.df [ppt.df$`Group Name`=="payc022521" & ppt.df$Time=="Before",c(1:25,98)] 
...
payc022521 <- rbind.data.frame(gender,race,age,grade)

#OAH 2: Dosage

#OAH 3: Fidelity
fidelity1 <- data.frame(table(fidelity.df$`MPC1 how`))
names(fidelity1) <- c("Implementation", "Freq")
fidelity2 <- data.frame(table(fidelity.df$`MPC2 how`))
names(fidelity1) <- c("Implementation", "Freq")
fidelity3 <- data.frame(table(fidelity.df$`MPC3 how`))
names(fidelity1) <- c("Implementation", "Freq")

fidelity <- rbind.data.frame(fidelity1,fidelity2,fidelity3)
...
library(sjPlot)
tab_df(payc022521,
       title = "Reach for Group PAYC022521",
       file = "PPT Demographics Table.doc")
tab_df(fidelity,
       title = "Lessons completed for Group PAYC022521",
       file = "FL Fidelity Table.doc")
tab_df(referrals,
       title = "Referrals for Group PAYC022521",
...
ggsave("PPT Experiences.png", width = 9, height = 2.5)

thoughtlkt <- likert(thoughts.df[,3:7], grouping = thoughts.df$Time)
plot(thoughtlkt, group.order = c("Before","After")) + ggtitle("Participants' Intended Behaviors")
ggsave("PPT Thoughts.png", width = 9, height = 6)
```
