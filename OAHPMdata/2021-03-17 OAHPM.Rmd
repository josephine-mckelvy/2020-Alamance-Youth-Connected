...
demographics.df$gender_cat <- apply(demographics.df[18:22], 1, function(x) {ifelse(sum(x) > 1, "Multigender", names(x[x != 0]))})
gender <- data.frame(table(demographics.df$gender_cat))
names(gender) <- c("Demographics","Freq")

demographics.df$race_cat <- apply(demographics.df[8:13], 1, function(x) {ifelse(sum(x) > 1, "Multiracial", names(x[x != 0]))})
OAHrace <- data.frame(table(demographics.df$race_cat,demographics.df$Hispanic))
names(OAHrace) <- c("Demographics","Freq")
...
payc022521 <- rbind.data.frame(gender,OAHrace,age,grade) 
