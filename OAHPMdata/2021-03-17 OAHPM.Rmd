...
#Referrals for every group in the reporting period (change values of the EBP dates):
qtref <- flqpr.df[flqpr.df$`MPC1 start` >="2021-01-01" & flqpr.df$`MPC1 start` <="2021-03-31",c(1:3,9:15,33,38,43)]

#Sessions completed as planned in the reporting period (change values of the EBP dates):
qtrfid <- flqpr.df[flqpr.df$`MPC1 start` >="2021-01-01" & flqpr.df$`MPC1 start` <="2021-03-31",c(1:3,34,39,44)]
...
referrals <- data.frame(type=c("SRH","MH","SA","PCP","EDU","VOC","VIOL"),count=c(srh,mh,sa,pcp,edu,voc,viol))
racenew <- data.frame(type=c("black", "multi"), count=(race))

library(sjPlot)
tab_df(payc022521, referrals,
