...
ta.df[30:60] <- lapply(ta.df[30:60], cleanIt) # GTO
ta.df[61:82] <- lapply(ta.df[61:82], cleanIt) # training topics

#If more than one applied, then sum > 1, or "Multiple"
ta.df$mainio <- apply(ta.df[17:25], 1, function(x) {ifelse(sum(x) > 1, "Multiple settings", names(x[x != 0]))})
...
ta.df$mainio <- ifelse(ta.df$mainio %in% c("X.1"),"ACHD",
                ...
                ifelse(ta.df$mainio %in% c("X.9"),"SABGC",
...
ta.df$setting <- ifelse(ta.df$mainio %in% c("ACHD"), 1, #ACHD = Clinic
           ifelse(ta.df$mainio %in% c("ACDSS","CHS"), 2, #ACDSS, CHS = Foster Care
           ifelse(ta.df$mainio %in% c("CGDC","CrossRoads","PAYC","Exchange","SABGC"), 3, #CGDC, CrossRoads, PAYC, Exchange, SABGC = Community
...
tabyio <- aggregate(x = year$hours,
          by= list(year$mainio),
          FUN=sum) 
```
- GTO
```{r}
ta.df$gto <- apply(ta.df[30:60], 1, function(x) {ifelse(sum(x) > 1, "Multiple steps", names(x[x != 0]))})
...
gto.df <- ta.df[ta.df$Date.of.Training.or.TA >= begin & ta.df$Date.of.Training.or.TA <= finish & ta.df$mainio == "CGDC", c(12:14,30:60,85:88)]
