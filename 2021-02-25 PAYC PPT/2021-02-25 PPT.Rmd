...
ppt.df[13] <- lapply(ppt.df[13],cleanIt)
...
#if group name is exact:
demographics.df <- ppt.df [ppt.df$`Group Name`=="payc" & ppt.df$Time=="Before",c(1:23,96)] 
experiences.df <- ppt.df [ppt.df$`Group Name`=="payc",c(1:3,24:44)]
thoughts.df <- ppt.df[ppt.df$`Group Name`=="payc" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1,3,91:95)] 
ksa.df <- ppt.df [ppt.df$`Group Name`=="payc" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1:3,59:90)] 
satisfaction.df <- ppt.df [ppt.df$`Group Name`=="payc" & ppt.df$Time=="After",c(1:3,45:58)] 
...
mrdumrace <- data.frame(Freq=colSums(demographics.df[8:11]))
mrdumhisp <- data.frame(Freq=colSums(demographics.df[13]))
...
race <- data.frame(Demographics=c("White","Black","Asian","Alaska Native", "Native Hawaiian","Multiracial", "Race not Reported","Hispanic"),Freq=c(0,9,0,0,0,1,0,0))
