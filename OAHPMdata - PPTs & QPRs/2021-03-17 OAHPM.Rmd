...
obs.df <- rawobs.df[2:nrow(rawobs.df),c(10,14:16,34)]
...
obs.df$`Obs io` <- factor(obs.df$`Obs io`,
                          levels = c(1,2,3,4,5,6,7,8),
                          labels = labelio)
```
...
demographics.df <- ppt.df [ppt.df$`Group Name`=="dreamcenter032421" & ppt.df$Time=="Before",c(1:25,98)] #9 Demographic variables
experlkt.df <- ppt.df [ppt.df$`Group Name`=="dreamcenter032421",c(26:31)] #6 Experience variables (likert)
expermrd.df <- ppt.df [ppt.df$`Group Name`=="dreamcenter032421",c(1,32:46)] #15 Experience options (MR dummy)
thoughts.df <- ppt.df[ppt.df$`Group Name`=="dreamcenter032421" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1,3,93:97)] #5 Thought variables (likert)
ksa.df <- ppt.df [ppt.df$`Group Name`=="dreamcenter032421" & ppt.df$Time=="Before" | ppt.df$Time=="After",c(1:3,61:92)] #16 KSA variables with 32 options
satisfaction.df <- ppt.df [ppt.df$`Group Name`=="dreamcenter032421" & ppt.df$Time=="After",c(1,47:57)] #11 Group variables on Post
...
fidelity.df <- flqpr.df[flqpr.df$`FL io` %like% "PAYC" & flqpr.df$`MPC1 start` >="2021-01-01" & flqpr.df$`MPC1 start` <="2021-03-31",c(3,34,39,44)]
...
race <- data.frame(table(demographics.df$race_cat))
...
mrdumlastsex <- data.frame(Freq=colSums(expermrd.df[10:23])) #need chart for this in 6.

#OAH 2: Dosage

#OAH 3: Fidelity for MPC Lessons; use sjPlot::tab_xtab instead?
fidelity1 <- data.frame(table(fidelity.df$`MPC1 how`))
names(fidelity1) <- c("Implementation", "Count")
fidelity2 <- data.frame(table(fidelity.df$`MPC2 how`))
names(fidelity2) <- c("Implementation", "Count")
fidelity3 <- data.frame(table(fidelity.df$`MPC3 how`))
names(fidelity3) <- c("Implementation", "Count")
...
experlkt.df <- na.omit(experlkt.df)
thoughts.df <- na.omit(thoughts.df)

library(likert)
experiencelkt <- likert(items=experlkt.df)
...
ggsave("DreamCenter032421 - Thoughts over time.png", width = 9, height = 4.5)
```
