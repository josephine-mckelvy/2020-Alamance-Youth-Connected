...
ptexperlkt.df <- ppt.df[ppt.df$`Group Name`==thisgroup & ppt.df$Time=="Before",c(26:31)] #6 Experience variables (likert)
...
ptexpermrd.df <- ppt.df[ppt.df$`Group Name`==thisgroup & ppt.df$Time=="Before",c(32,34:45)] #13 Experience options (MR dummy excluding "Other method" (text))
...
###########################
ptthoughts.df <- ppt.df[ppt.df$`Group Name`==thisgroup,c(1,3,93:97)] #5 Thought variables (likert) for Pre AND Post
...
ptksa.df <- ppt.df[ppt.df$`Group Name`==thisgroup,c(1:3,61:92)] #16 KSA variables with 32 options
...
mpcgrp <- "dreamcenter062121"

mpc1 <- flqpr.df[flqpr.df$`FL grp`==mpcgrp,c(3,32:36)]
mpc2 <- flqpr.df[flqpr.df$`FL grp`==mpcgrp,c(3,37:41)]
mpc3 <- flqpr.df[flqpr.df$`FL grp`==mpcgrp,c(3,42:46)]
...
org <- "DreamCenter" #or "ACDSS", "CHS", "PAYC", "SA"
...
tab_df(suppsrvs,
       title = "Referrals for: DreamCenter",
       file = "DreamCenter - Referrals Table.doc")
```
# Alamance Cares
```{r}
